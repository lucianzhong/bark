...
std::pair<AccelerationLimits, AccelerationLimits> ConvertRestrictions(
  ...
  if (history.size() >= 1) {
    ...
    last_action = (history.end() - 1)->second;
  } else {
    last_state = observed_world.CurrentEgoState();
    last_action = Continuous1DAction(0.0);
  }
  ...
  if (switch_off_lat_limits_on_road_x &&
      Collide(ego_polygon, roadx_polygon)) {
  ...
  return std::make_pair(acc_lim_street_cs, acc_lim_vehicle_cs);
}
#endif
...
Trajectory BehaviorRSSConformant::Plan(
  ...
  if (rss_evaluator) {
    ...
    const auto limits = ConvertRestrictions(min_planning_time, acc_restrictions_, observed_world, 
                      switch_off_lat_limits_on_road_x_, roadx_polygon_, rss_vlat_threshold_);
    SetAccelerationLimitsStreetCs(limits.first);
    SetAccelerationLimitsVehicleCs(limits.second);
  }
#endif
